...
def get_session_language(
    ...
    try:
        ...
        accept_languages = parse_accept_language(session_language)
        found_locales = []
        for language in accept_languages:
            language_close_matches = get_close_matches(
                language.header_name,
                app_locales,
                n=1,
            )
            if len(language_close_matches) > 0:
                language.locale_name = language_close_matches[0]
                found_locales.append(language)
        if len(found_locales) < 1:
            return "en"
        found_locales.sort(key=lambda locale: locale.q, reverse=True)
        return found_locales[0].locale_name
    except Exception as e:
...
class Language:
    def __init__(self, header_name: str, q: float, locale_name: Optional[str] = None):
...
def parse_accept_language(accept_language_header: str) -> List[Language]:
